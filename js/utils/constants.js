  ...
  const DEVLOG_DATA = [
    {
      ...
      "title": "📂 AI 규칙 설정법: 폴더 배치·읽는 순서·우선순위 (v1.6)",
      "badge": "최신 버전 ✨",
      "badgeColor": "#20c997",
      "summary": "공통 규칙과 프로젝트 규칙을 어디에 두고, AI가 어떻게 읽는지. 규칙 충돌과 다른 PC 설정까지 예시로 익히기.",
      "details": [
        "📂 <strong>1. 공통 규칙은 부모 폴더에, 프로젝트는 나란히</strong><br>예시 기준 폴더: <code>C:/Users/JY/Desktop/vibe_coding</code><pre style=\"white-space:pre-wrap;overflow-wrap:anywhere;\">vibe_coding/\n├─ AGENTS.md          모든 프로젝트의 공통 규칙\n├─ ToDoList/          투두리스트 프로젝트\n│  ├─ AGENTS.md       Codex에 규칙 위치 안내\n│  ├─ GEMINI.md       Gemini에 규칙 위치 안내\n│  ├─ PROJECT_RULES.md  이 프로젝트 전용 규칙\n│  ├─ index.html\n│  └─ js/\n└─ Myportfolio/       별도 프로젝트\n   ├─ AGENTS.md\n   ├─ GEMINI.md\n   └─ PROJECT_RULES.md</pre>새 프로젝트는 ToDoList 안에 넣지 말고, 같은 부모 폴더 아래에 나란히 만듭니다. 공통 규칙 본문은 한 곳에서 관리합니다.",
        "📝 <strong>2. 어떤 내용을 어디에 적을까?</strong><br>• <strong>공통 AGENTS.md</strong>: 답변 언어, 작업 방식처럼 여러 프로젝트에 똑같이 적용할 내용<br>• <strong>PROJECT_RULES.md</strong>: 사용 기술, 실행·검사 명령, 보호할 데이터, 배포 조건<br>• <strong>프로젝트의 AGENTS.md·GEMINI.md</strong>: 위 두 파일을 읽으라는 짧은 안내<br>연결문 예시:<pre style=\"white-space:pre-wrap;overflow-wrap:anywhere;\">이 파일이 있는 폴더를 기준으로\n../AGENTS.md를 먼저 읽고,\n./PROJECT_RULES.md를 추가로 읽는다.\n충돌하는 프로젝트 세부사항은 프로젝트 규칙을 우선한다.</pre><code>../</code>는 한 단계 위 폴더, <code>./</code>는 현재 파일이 있는 폴더입니다. PROJECT_RULES.md는 이름만으로 자동 인식되는 파일이 아니므로 읽을 위치를 알려줘야 합니다.",
        "🔎 <strong>3. Codex가 규칙을 읽는 순서</strong><br>기본 설정: <strong>개인 설정 → 프로젝트 최상위 → 현재 작업 폴더까지의 하위 규칙</strong><br>① <code>C:/Users/JY/.codex/AGENTS.md</code><br>② <code>ToDoList/AGENTS.md</code><br>③ 하위 폴더에서 시작했다면, 그 경로에 있는 AGENTS.md도 추가<br>부모 <code>vibe_coding</code>이 Git 프로젝트 밖이면 자동 탐색에 포함된다고 가정하지 않습니다. 위 연결문으로 공통 규칙을 읽게 합니다. 폴더 안의 모든 규칙을 무조건 읽는 것도 아닙니다. <a href=\"https://learn.chatgpt.com/docs/agent-configuration/agents-md\" target=\"_blank\" rel=\"noopener noreferrer\">Codex 공식 읽기 순서</a>",
        "🔄 <strong>4. 규칙이 겹치면 무엇이 우선일까?</strong><br>Codex가 읽은 규칙 중 <strong>충돌하는 부분은 현재 작업 폴더에 더 가까운 규칙이 우선</strong>입니다.<br>예시:<br>• 공통: “한국어로 답변, 코드 들여쓰기 2칸”<br>• 프로젝트: “코드 들여쓰기 4칸”<br>→ 적용 결과: <strong>한국어로 답변 + 들여쓰기 4칸</strong><br>공통 규칙 전체가 사라지거나 파일이 덮어써지는 것은 아닙니다. 같은 폴더에 <code>AGENTS.override.md</code>가 있으면 그 폴더의 AGENTS.md 대신 선택되므로, 예상과 다르면 이 파일도 확인하세요. 시스템·도구의 권한 제한은 규칙 파일로 바꿀 수 없습니다.",
        "🤖 <strong>5. Gemini·Antigravity도 같은 원본을 읽게 하려면?</strong><br>Antigravity의 <strong>Customizations → Rules</strong>에서 연결합니다.<br>• 전역 규칙: <code>~/.gemini/GEMINI.md</code>에서 공통 AGENTS.md의 전체 경로 안내<br>• 프로젝트 규칙: <code>.agents/rules</code>에 해당 PROJECT_RULES.md를 읽는 안내를 두고 <strong>Always On</strong>으로 설정<br><code>~</code>는 사용자 폴더이며, 이 PC에서는 <code>C:/Users/JY</code>입니다. 프로젝트 최상위에 GEMINI.md를 놓는 것만으로 모든 Gemini 도구가 똑같이 자동 적용한다고 생각하면 안 됩니다. 사용하는 도구의 Rules 설정을 확인하세요. <a href=\"https://antigravity.google/docs/rules-workflows\" target=\"_blank\" rel=\"noopener noreferrer\">Antigravity 공식 설정법</a>",
        "💻 <strong>6. 새 프로젝트·다른 PC에서 바로 쓰는 순서</strong><br>① 부모 폴더에 공통 AGENTS.md를 준비합니다.<br>② 프로젝트 폴더에 전용 PROJECT_RULES.md와 연결 파일을 둡니다.<br>③ PC별 Codex·Antigravity 설정에서 공통 파일 경로를 맞춥니다.<br>④ 새 대화를 시작해 “읽은 규칙 파일의 경로와 우선순위를 알려줘”로 확인합니다.<br><strong>실전 팁:</strong> Git에 포함한 프로젝트 규칙만 다른 PC로 따라갑니다. 부모 폴더의 공통 규칙과 개인 AI 설정은 따로 준비하세요. 규칙 본문을 여러 곳에 복사하지 말고 경로로 연결하면, 한 곳만 고쳐도 같은 원본을 읽게 할 수 있습니다."
      ]
    },
    {
      version: 'v1.5',
      ...
      details: [
        '🌐 <strong>[전역 규칙 일원화] 6대 공통 개발 원칙 확립</strong> (IDE 전역 설정)<br>• 요청 관련 코드만 집중 확인 (토큰 소모 및 무의미한 전체 반복 분석 차단)<br>• 작업 범위 내 명백한 버그·보안 위험 발견 시 능동적 사전 알림<br>• 기존 데이터·설정·주석 임의 삭제 금지 & 대규모 변경 사전 승인 필수<br>• 위험도별 최소 충분 검증 & 전체 심층 분석은 사용자 요청 시에만 수행',
        '📂 <strong>[규칙 파일 역할 분리]</strong><br>• <strong>AGENTS.md</strong>: 에이전트 행동 및 작업 수칙(How to work) 전담<br>• <strong>GEMINI.md</strong>: ToDoList 핵심 데이터·키·암호화(What to protect) 전담<br>• 규칙 중복과 에이전트 판단 혼선을 원천 차단',
        '🎯 <strong>[정량적 검증 기준 정립] ➔ AGENTS.md 7번 항목</strong><br>• 🟢 <strong>LOW</strong>: *.css, *.md, 단순 HTML ➔ 변경된 화면 스타일 확인<br>• 🟡 <strong>NORMAL</strong>: 화면 렌더링(render*), 버튼 이벤트 ➔ 연결 흐름 검증<br>• 🔴 <strong>HIGH</strong>: 로컬 저장소, Firebase, 암호화, 데이터 모델 ➔ 저장/복원 철저 검증<br>• 🟣 <strong>FULL</strong>: 대규모 리팩터링 또는 명시적 /verify-full 요청 시',
    ...
    },
    {
      ...
      date: '2026-09-16',
      dateFormatted: '2026년 9월 16일 (수)',
      title: '🛡️ 로컬 데이터 보호·클라우드 동기화 안정화 (v1.4)',
      badge: '안정화 완료 ✅',
      badgeColor: '#4dabf7',
      summary: '메모의 로컬 저장 확인, 미전송 변경 보존, 수동 동기화와 AI 노트 수신 개선 및 화면 처리 경량화',
      details: [
        '💾 읽기·파싱 실패 시 빈 데이터 덮어쓰기를 차단하고, 메모는 로컬 저장 성공을 확인한 뒤 입력창을 비우도록 개선',
        '📤 localSync에 미전송 변경을 보관하여 재실행 후에도 유지하고, 서버 저장 확인 전에는 미전송 상태를 해제하지 않도록 보호',
        '🔄 수동 동기화 버튼과 완료·필요·진행·실패 상태 표시 추가. 로그인 성공과 데이터 동기화 성공을 구분',
        '🤖 다른 항목이 충돌해도 안전하게 추가할 수 있는 새 메모·AI 노트는 수신. 같은 노트의 충돌 원문과 삭제 기록은 보존',
        '🔐 계정 확인 통신 실패를 신규 등록으로 처리하지 않도록 수정하고, 로컬 전용 관리정보는 암호화·업로드 대상에서 제외',
        '📒 가계부 화면을 열 때 기기별 거래 원본으로 기존 월별 금액을 덮어쓰던 경로 차단',
        '⚡ 변경 없는 동기화의 반복 계산, 숨겨진 화면 렌더링, 중복 메뉴 갱신과 AI 목록의 불필요한 가공을 줄임',
        '🧪 가짜 저장소·서버로 통신 실패, 재실행, 충돌, 미전송 보존 및 모바일 화면 검증. 기존 저장 키·메모 형식·암호화 방식 유지'
      ]
    },
    {
      version: 'v1.3.9',
