...
상태: 4차 안전성 수정·5개 화면 함수 분리·검증 및 최종 재검토 완료. 운영 데이터 접근·main 병합·배포 없음. 검토 문서 커밋을 포함한 GitHub 복구 브랜치·고정 태그는 아래 절차로 게시하고 원격 해시를 확인한다. 5차는 계획만 작성했다.

## 2026-09-19 최종 재검토와 복구 지점

- 검토한 애플리케이션 코드: `d1b8f08`(4차 완료). 이번 재검토에서 앱 코드 수정은 없으며, 문서만 추가·갱신했다.
- `0da98a0..d1b8f08` 변경 중 중앙 병합의 사이트/폴더 삭제 판정, 로컬 저장 성공 후 메모리 반영, 폴더 삭제와 사이트 이동의 단일 저장, 폼의 실패 처리, 두 HTML의 로딩 순서를 다시 확인했다. 확인한 범위에서 추가로 수정해야 할 4차 회귀는 발견하지 않았다.
- Node 관련 검사 158개를 다시 실행해 모두 통과했다. `sites-ui.cjs`의 기존 40개 화면 기준, `ai-study-ui.cjs`의 기존 32개 화면 기준도 모두 통과했다. 새 기준을 생성해 비교를 대체하지 않았다.
- 합성 자료로 사이트/폴더 생성·수정·삭제 6개 경로의 성공 저장을 추가 검사했다. 관련 없는 기존 필드와 알 수 없는 기존 확장 필드는 변경되지 않았고 실제 네트워크 요청은 없었다. 이는 해당 로컬 변경 경로의 검사이며 모든 구버전·전체 클라우드 자료의 검증을 의미하지 않는다.
- 문법 검사, diff 공백 검사, 두 HTML의 바이트 동일성, 개인 복구 파일·scratch의 Git 제외 상태를 확인했다.
- GitHub 복구 브랜치: `codex/refactor-phase-4`. 고정 annotated 태그: `codex/phase-4-reviewed-20260919`. 둘 다 검토·5차 계획 문서까지 포함한 동일 커밋을 가리키도록 게시한다. 이 태그는 향후 이동시키지 않는다.
- 게시 후 `git ls-remote`에서 브랜치 커밋 및 태그의 역참조 커밋을 로컬 `HEAD`와 대조한다. `main`은 게시 전 `b60a3c68a1b09f92ed0453953f61fb6bc0ec9f6b`이며 게시 후 같은지 확인한다. 브랜치·태그 게시를 운영 배포로 취급하지 않는다.
- 재검토 로그와 합성 재현 스크립트는 Git에서 제외한 `scratch/phase4-review-*`에 보관했다. 실제 브라우저 프로필, 개인 복구 JSON, 운영 Firebase, 파일보관함 원문은 열거나 수정하지 않았다.
- 소스 복구는 태그에서 새 브랜치/별도 체크아웃을 만드는 방식으로 시작한다. 현재 작업 폴더의 강제 초기화, 개인 데이터 초기화, 오래된 서버 자료의 덮어쓰기는 복구 방법에 포함하지 않는다.

GitHub 복구 지점은 **소스코드 백업**이다. `localStorage`·`IndexedDB`·Firebase·기기별 은행 원본까지 백업됐다고 간주하지 않는다. 향후 실제 운영 적용 전에는 각 자료의 별도 백업 범위와 복원 가능 여부를 확인한다. 현 단계의 검사 통과만으로 모든 실제 기기의 무손실을 보장하지 않는다.
